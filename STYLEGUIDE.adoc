...
. In Asciidoc, use exactly one sentence per line.
A long sentences should be on one line, even if this line is longer than 120 characters as this will encourage you to use shorter sentences.
A short sentence should not get friends on the same line.
When you use one sentence per line, diffs are easier as only the affected sentences are modified.
Asciidoc will make the whole a paragraph anyway.

. When updating documentation, only modified lines should have the conventions applied.
Applying conventions to unmodified lines must only take place in a dedicated commit.
